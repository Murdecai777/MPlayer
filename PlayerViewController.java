...
public class PlayerViewController implements Initializable {
    ...
    Boolean CREATED_EQ = false, CREATED_LYRIC = false;


    Analizer analizer;
    Lyrics lyric;
    String songName, trackList  = MPlayer.choosedList;
    ...
    private void getAllMusic(File song) {
        if (song.isDirectory()) {
            ...
            if (song.getName().toLowerCase().endsWith(".mp3") || song.getName().toLowerCase().endsWith(".wav")
                ...
                music.setName(song.getName().substring(0, song.getName().length() - 4));
    ...
    public void playClick(Music music) {
       ...
       // if(MPlayer.download != null){
       //     MPlayer.download.start();
       // }
        mediaPlayer.play();
    ...
    public void stopMusic() throws InterruptedException {
     ...
     /*   if(MPlayer.download != null){
            System.out.println("download != null");
            if(MPlayer.download.isAlive()){
                System.out.println("download is Alive!");
                MPlayer.downloadFiles("");
                MPlayer.download.interrupt();
            }
        } */
    }
    ...
    public void openLyric() throws IOException{
        if(CREATED_LYRIC == false){
            lyric = new Lyrics();
            CREATED_LYRIC = true;
            lyric.songName = songName;
            lyric.strt();
        }else{
            lyric.closeLyricStage();
            CREATED_LYRIC = false;
            lyric = null;
        }
    ...
    public void chooseList(){
        ...
        if(MPlayer.listWasChanged){
            trackList  = MPlayer.choosedList;
